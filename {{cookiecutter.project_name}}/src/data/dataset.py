# -*- coding: utf-8 -*-
import logging
import urllib.request
from pathlib import Path
from zipfile import ZipFile

import click
from dotenv import find_dotenv, load_dotenv


@click.command()
@click.argument('uri', type=click.STRING)
def download(uri: str) -> None:
    """Download the given uri (zip file).

    Args:
        uri (str): _description_
    """
    logger = logging.getLogger(__name__)
    logger.info('Download the dataset')

    # create destination dirs
    destination = project_dir / 'data' / 'raw'
    destination.mkdir(exist_ok=True, parents=True)

    # download the file
    urllib.request.urlretrieve(uri, destination / "original.zip")


def unzip() -> None:
    """Unzip the downloaded file."""
    logger = logging.getLogger(__name__)
    logger.info('Download the dataset')

    # define the destination
    destination = project_dir / 'data' / 'raw'

    # extract zip
    zip_file = ZipFile(destination / "original.zip")
    zip_file.extractall(destination)


@click.command()
@click.argument('input_filepath', type=click.Path(exists=True))
@click.argument('output_filepath', type=click.Path())
def make(input_filepath, output_filepath) -> None:
    """Run data processing scripts to turn raw data from (./raw) into cleaned data ready to be analyzed (./processed).

    Args:
        input_filepath (_type_): _description_
        output_filepath (_type_): _description_
    """
    logger = logging.getLogger(__name__)
    logger.info('making final data set from raw data')


if __name__ == '__main__':
    log_fmt = '%(asctime)s - %(name)s - %(levelname)s - %(message)s'
    logging.basicConfig(level=logging.INFO, format=log_fmt)

    # not used in this stub but often useful for finding various files
    project_dir = Path(__file__).resolve().parents[2]

    # find .env automagically by walking up directories until it's found, then
    # load up the .env entries as environment variables
    load_dotenv(find_dotenv())
